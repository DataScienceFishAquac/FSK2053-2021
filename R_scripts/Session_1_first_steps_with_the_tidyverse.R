...
# Check if the file is now in your working directory using the `file.exists` function:
...
# They do not anymore. It now depends on what value has been set for default.stringsAsFactors()
...
# The file might have a multiline header, missing cells, or it might use an unexpected [encoding]
...
# Now that we are done with the example we will remove the example file we copied over 
...
# However, if we want to represent several years in the plot, we cannot do that directly
...
# After the first step in the data analysis process, importing data, a common next step is
...
# We have to specify the names of the new columns that will be created with "variable" and "value"
# And the third argument will be the names of the columns that will be gathered into tidy format
tidy_data <- wide_data %>% gather("variable","value",all_of(years))

# The first argument sets the column/variable name that will hold the variable that is currently kept
# in the wide data column names. We can name it anything. 
# The second argument sets the column/variable name that will hold the values in the column cells.
tidy_data

# A more practical solution would be to call them `year` and `fishing hours` like this:
...
# Since the shape palette is no good for more than 6 values, we will change colors and shape
tidy_data %>% ggplot(aes(tonnage_gt_gfw, fishing_hours, color = year)) +
  geom_point(aes(shape = flag_gfw))

# Another way to write this code is to specify which columns will **not** be gathered 
...
# We can create new columns (mutate) or overwrite existing ones using the function mutate()
...
# We can use the function transmute to create a whole new tibble with new variables.
...
# `pivot_longer` is another way to produce a tidy (`longer`) dataset from a wide_data table.
...
tidy_data <- wide_data %>% pivot_longer(all_of(years),names_to = "year", values_to = "fishing_hours") %>% 
  mutate(year = gsub("fishing_hours_","",year))  %>% drop_na()
...
new_wide_data <- tidy_data %>% pivot_wider(names_from = year, values_from=fishing_hours)
...
# Encoding multiple variables in a column name is such a common problem that the `readr` package includes a function 
# to separate these columns into two or more. 
# The `separate` function takes three arguments: (1) the name of the column to be separated, 
# (2) the names to be used for the new columns and (3) the character that separates the variables.
...
# However, we run into a problem. Note that we receive the warning `Expected 2 pieces. Additional pieces discarded in 112 rows:`
...
# We could add a third column to catch this and let the `separate` function know which column to fill in with NA, when there is no third value.
